  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#FFF5F7', paddingBottom: '100px', fontFamily: 'sans-serif' }}>
      <header style={{ backgroundColor: 'white', padding: '20px', borderRadius: '0 0 30px 30px', textAlign: 'center', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <button onClick={() => { const p = prompt("Senha:"); if(p==="mari123") setShowAdmin(true) }} style={{ position: 'absolute', right: '20px', top: '25px', opacity: 0.2, border: 'none', background: 'none' }}>⚙️</button>
      </header>

      <main style={{ padding: '20px' }}>
        {products.length === 0 && <p style={{ textAlign: 'center', color: '#FF85A2', marginTop: '50px' }}>Carregando doces da Mari...</p>}
        <div style={{ display: 'grid', gap: '15px' }}>
          {products.map(p => (
            <div key={p.id} style={{ backgroundColor: 'white', borderRadius: '20px', padding: '15px', display: 'flex', gap: '15px', alignItems: 'center', boxShadow: '0 4px 10px rgba(0,0,0,0.03)' }}>
              <img src={p.image || 'https://placeholder.com'} style={{ width: '80px', height: '80px', borderRadius: '15px', objectFit: 'cover' }} />
              <div style={{ flex: 1 }}>
              ...
              <button onClick={() => setCart([...cart, p])} style={{ backgroundColor: '#FF85A2', color: 'white', border: 'none', width: '35px', height: '35px', borderRadius: '10px', fontSize: '20px', fontWeight: 'bold' }}>+</button>
            </div>
          ))}
        </div>
      </main>
      ...
      {cart.length > 0 && (
        <div style={{ position: 'fixed', bottom: '20px', left: '20px', right: '20px', zIndex: 20 }}>
          <button onClick={finalizeOrder} style={{ width: '100%', backgroundColor: '#2D2D2D', color: 'white', padding: '20px', borderRadius: '20px', border: 'none', fontWeight: 'bold', fontSize: '16px', boxShadow: '0 10px 20px rgba(0,0,0,0.2)' }}>
      ...
      {showAdmin && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', padding: '30px', zIndex: 100, display: 'flex', alignItems: 'center' }}>
          <div style={{ backgroundColor: 'white', width: '100%', borderRadius: '25px', padding: '25px', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <input placeholder="Nome" style={{ width: '100%', padding: '12px', marginBottom: '10px', borderRadius: '10px', border: '1px solid #ddd' }} onChange={e => setNewProduct({...newProduct, name: e.target.value})} />
            <input placeholder="Preço" type="number" style={{ width: '100%', padding: '12px', marginBottom: '10px', borderRadius: '10px', border: '1px solid #ddd' }} onChange={e => setNewProduct({...newProduct, price: e.target.value})} />
            <input placeholder="Link da Foto" style={{ width: '100%', padding: '12px', marginBottom: '20px', borderRadius: '10px', border: '1px solid #ddd' }} onChange={e => setNewProduct({...newProduct, image: e.target.value})} />
            <button onClick={() => { if(!newProduct.name || !newProduct.price) return alert("Preencha tudo!"); push(ref(db, 'products'), {...newProduct, price: parseFloat(newProduct.price)}); alert("Salvo!"); }} style={{ width: '100%', backgroundColor: '#FF85A2', color: 'white', padding: '15px', borderRadius: '15px', border: 'none', fontWeight: 'bold' }}>SALVAR NOVO DOCE</button>

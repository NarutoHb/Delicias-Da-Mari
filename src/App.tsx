    ...
    <div style={{ minHeight: '100vh', paddingBottom: '120px' }}>
            ...
            <div style={{ display: 'flex', overflowX: 'auto', gap: '15px', paddingBottom: '10px' }} className="no-scrollbar">
              {products.filter(p => p.category === cat).map(p => (
                <div key={p.id} style={{ minWidth: '200px', backgroundColor: 'white', borderRadius: '25px', padding: '15px', boxShadow: '0 8px 20px rgba(0,0,0,0.03)' }}>
                  <img src={p.image || 'https://unsplash.com'} style={{ width: '100%', height: '130px', borderRadius: '20px', objectFit: 'cover' }} />
      ...
      {showAdmin && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'white', padding: '30px', zIndex: 100, overflowY: 'auto' }}>
          ...
          <button onClick={() => { if(!newProduct.name || !newProduct.price) return alert("Preencha tudo!"); push(ref(db, 'products'), {...newProduct, price: parseFloat(newProduct.price)}); alert("Salvo!"); }} style={{ width: '100%', backgroundColor: '#FF85A2', color: 'white', padding: '18px', borderRadius: '15px', border: 'none', fontWeight: 'bold' }}>SALVAR NOVO DOCE</button>

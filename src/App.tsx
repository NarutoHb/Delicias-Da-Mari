...
// Configuração Firebase Revisada
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#FFF5F7', paddingBottom: '100px', fontFamily: 'sans-serif' }}>
      ...
      <main style={{ padding: '20px' }}>
        {products.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#FF85A2', marginTop: '50px' }}>Carregando doces ou cardápio vazio... 🎀</p>
        ) : (
          <div style={{ display: 'grid', gap: '15px' }}>
            {products.map(p => (
              <div key={p.id} style={{ backgroundColor: 'white', borderRadius: '20px', padding: '15px', display: 'flex', gap: '15px', alignItems: 'center', boxShadow: '0 4px 10px rgba(0,0,0,0.03)' }}>
                <img src={p.image || 'https://placeholder.com'} style={{ width: '80px', height: '80px', borderRadius: '15px', objectFit: 'cover' }} alt={p.name} />
                <div style={{ flex: 1 }}>
          ...
          <div style={{ backgroundColor: 'white', width: '100%', borderRadius: '25px', padding: '25px', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <button 
              onClick={() => { 
                ...
                alert("Salvo!"); 
                setNewProduct({ name: '', price: '', category: 'Bolos de Pote', image: '' });
              }} 
              style={{ width: '100%', backgroundColor: '#FF85A2', color: 'white', padding: '15px', borderRadius: '15px', border: 'none', fontWeight: 'bold' }}
            >SALVAR DOCE</button>
            <button onClick={() => setShowAdmin(false)} style={{ width: '100%', marginTop: '20px', background: 'none', border: 'none', color: '#999' }}>Fechar Painel</button>
          </div>
        </div>
      )}
    </div>

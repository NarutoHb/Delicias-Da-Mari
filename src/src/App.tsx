      ...
      {showAdmin && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 p-6 flex items-center justify-center">
          <div className="bg-white w-full max-w-sm rounded-[30px] p-8 shadow-2xl">
            ...
            <input placeholder="Preço (Ex: 15.00)" type="number" className="w-full border p-3 mb-2 rounded-xl outline-none" onChange={e => setNewProduct({...newProduct, price: e.target.value})} />
            ...
            <button onClick={() => { push(ref(db, 'products'), {...newProduct, price: parseFloat(newProduct.price)}); setShowAdmin(false); }} className="w-full bg-[#FF85A2] text-white font-bold py-4 rounded-xl mt-2">SALVAR DELÍCIA</button>
            <button onClick={() => setShowAdmin(false)} className="w-full text-gray-400 mt-4 text-sm">Cancelar</button>
          </div>
        </div>
      )}
    </div>
